...
def txt_to_list(txt_file : pathlib.Path ) -> list[str]: # FIXED
    ...
    try:
        ...
        # to avoid unreadable character use a default encoding depending on your pc? or better to use utf-8
    ...
    #for completeness eve if the test doesn't seems to care
...
def default_file_path() -> pathlib.Path: # FIXED 
    ...
    if os.path.exists(txt_file_default):  # if exists write it if blank
        with open(txt_file_default, 'r+', encoding='utf-8') as b: 
            blanked= b.read() ==  ''
            if blanked:  # to avoid the overwrite of the file if not empty
                b.write(content) 
            b.close()
